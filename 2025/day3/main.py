...
def find_max_joltage(bank: str) -> int:
  n = len(bank)
  max_suffix = [int(char) for char in bank]
...
# answer = sum([find_max_joltage(bank) for bank in data])
# print(answer)
...
def find_max_joltage2(bank: str) -> int:
  # Monotonic stack
  n = len(bank)
  stack = []
  b = [int(char) for char in bank]

  for i, num in enumerate(b):
    while stack and stack[-1] < num and len(stack) - 1 + n - i >= 12:
      stack.pop()
...
# print(find_max_joltage2('811111111111119'))

answer = sum([find_max_joltage2(bank) for bank in data])
print(answer)
