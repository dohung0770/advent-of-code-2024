...
def rotate(shape: list[str]) -> list[str]:
  ...
  new_shape = [['.'] * m for _ in range(n)]
  for i in range(m):
    for j in range(n):
      new_shape[j][m - 1 - i] = shape[i][j]

  return new_shape

def get_area(shape: list[str]) -> int:
  m, n = len(shape), len(shape[0])
  area = 0
  for i in range(m):
    for j in range(n):
...
def can_put(region: list[list[str]], shape: list[str], r: int, c: int) -> bool:
  ...
  for i in range(m):
    for j in range(n):
      if shape[i][j] == '#' and region[r + i][c + j] != '.':
        return False
      
  return True

def mark_shape(region: list[list[str]], shape: list[str], r: int, c: int) -> bool:
  m, n = len(shape), len(shape[0])
...
def unmark_shape(region: list[list[str]], shape: list[str], r: int, c: int) -> bool:
  m, n = len(shape), len(shape[0])
...
def part1(shapes: list[list[list[str]]], regions: list[list[int]], quantity: list[list[int]]):
  n = len(shapes)
  area = [0] * n # area of each shape
  rotations = [[shape] for shape in shapes] * n # all rotations of each shape

  for i, shape in enumerate(shapes):
    area[i] = get_area(shape)
    while len(rotations[i]) < 4:
      rotations[i].append(rotate(rotations[i][-1]))


  def solve(region: list[int], req: list[int]) -> bool:
    M, N = len(region), len(region[0])
    ...
    def dfs(idx: int) -> bool:
      ...
      for shape in rotations[idx]:
        for i in range(M):
          for j in range(N):
            if region[i][j] == '#':
              continue

            if can_put(region, shape, i, j):
              mark_shape(region, shape, i, j)
              req[idx] -= 1

              if dfs(idx) is True:
                return True

              unmark_shape(region, shape, i, j)
              req[idx] += 1
  ...
  cnt = 0
  for i, (width, height) in enumerate(regions):
    total_shape_area = 0
    for j, amount in enumerate(quantity[i]):
      total_shape_area += amount * area[j]

    # There won't be enough space to put all the gifts
    if total_shape_area > width * height:
      continue

    if solve([['.'] * width for _ in range(height)], quantity[i]):
      print(i)
      cnt += 1
...
def main():
  ...
  for line in sample_data:
    ...
    if line.find('x') != -1:
      area, req = line.split(': ')
      regions.append(list(map(int, area.split('x'))))
      quantity.append(list(map(int, req.split(' '))))
    elif line.find('#') != -1 or line.find('.') != -1:
      shapes[-1].append(list(line))

  shapes.pop()

  part1(shapes, regions, quantity)
