...
def parse_data(data: list[str]):
  '''
  Parse input data into the list of (indicator light diagrams, button wiring schematics and joltage requirements)
  
  :param data: data from input file
  :type data: list[str]
  '''

  for line in data:
    diagram, *buttons, joltage_req = line.split(' ')
    indicator_light_diagrams.append(diagram[1:-1])
    button_wiring_schematics.append([list(map(int, part[1:-1].split(','))) for part in buttons])
    joltage_requirements.append(list(map(int, joltage_req[1:-1].split(','))))

def min_presses(light_diagram: str, buttons: list[list[int]]) -> int:
  n = len(light_diagram)
  max_button = 0
  for b in buttons:
    max_button = max(max_button, max(b))

  desired = 0
  for i in range(n):
    if light_diagram[i] == '#': # should be on
      desired |= (1 << i)

  # precompute the mask for each button sequence press
  press_pattern = []
  for b in buttons:
    mask = 0
    ...
    press_pattern.append(mask)

  # BFS
  queue = deque([(0, 0)]) # (number of presses, current mask)
  visited = set([0])

  while queue:
    presses, curr_mask = queue.popleft()
    if curr_mask == desired:
      return presses
    
    for pattern in press_pattern:
      next_mask = curr_mask ^ pattern

      if next_mask not in visited:
        visited.add(next_mask)
        queue.append((presses + 1, next_mask))
...
def part1():
  '''
  Returns the total minimum number of presses to make the active light diagrams match the indicator light diagram on each line of input data

  '''
  parse_data(data)

  tot = 0
  for i in range(len(indicator_light_diagrams)):
    cnt = min_presses(indicator_light_diagrams[i], button_wiring_schematics[i])
    tot += cnt
...
def join(nums: list[int], separator: str = '.') -> str:
  return separator.join([str(val) for val in nums])

def min_presses2(buttons: list[list[int]], joltage_requirements: list[int]) -> int:
  queue = deque([(joltage_requirements, 0)]) # (current joltage levels, number of presses)
  visited = set([join(joltage_requirements)])

  while queue:
    curr_joltage_levels, presses = queue.popleft()
    print(curr_joltage_levels, presses)

    if max(curr_joltage_levels) == 0:
      return presses
    ...
    for nums in buttons:
      ...
      for idx in nums:
        if curr_joltage_levels[idx] == 0:
          sub = -1
          break
        
        sub = min(sub, curr_joltage_levels[idx])

      if sub > 0:
        next_levels = [curr for curr in curr_joltage_levels]
        for idx in nums:
          next_levels[idx] -= sub

        state = join(next_levels)
        if state not in visited:
          queue.append((next_levels, presses + 1))
          visited.add(state)
...
def part2():
  '''
  Returns the total fewest number of presses to configure joltage level
    to matches the joltage requirements for all of the machines
  '''

  parse_data(data)
  print('n = ', len(joltage_requirements))
  tot = 0

  for i in range(1, len(joltage_requirements)):
    cnt = min_presses2(button_wiring_schematics[i], joltage_requirements[i])
    print(i, cnt)
    if cnt >= 0:
      tot += cnt
